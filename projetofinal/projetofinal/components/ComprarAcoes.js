...
class ComprarAcoes extends React.Component {
  ...
  confirmarCompra() {
    ...
    Alert.alert(
      ...
      [
        ...
        { text: '✅ Confirmar', onPress: () => { Vibration.vibrate([0, 100, 80, 100]); this.executarCompra(qtd, total, acao); } }
      ]
    );
  }
  ...
  executarCompra(qtd, total, acao) {
    ...
    const transacao = {
      tipo: 'COMPRA', ticker: acao.ticker, nomeEmpresa: acao.nome,
      quantidade: qtd, precoUnitario: parseFloat(acao.preco),
      totalGasto: total, data: new Date().toISOString(),
    };
    firebase.database().ref(`usuarios/${uid}/transacoes`).push(transacao)
      .then(() => firebase.database().ref(`usuarios/${uid}/carteira`).orderByChild('ticker').equalTo(acao.ticker).once('value'))
      .then(snapshot => {
        if (snapshot.val()) {
          const chave = Object.keys(snapshot.val())[0];
          const existente = snapshot.val()[chave];
          const novaQtd = existente.quantidade + qtd;
          const novoPrecoMedio = ((existente.precoMedio * existente.quantidade) + (parseFloat(acao.preco) * qtd)) / novaQtd;
          return firebase.database().ref(`usuarios/${uid}/carteira/${chave}`).update({ quantidade: novaQtd, precoMedio: parseFloat(novoPrecoMedio.toFixed(2)) });
        } else {
          return firebase.database().ref(`usuarios/${uid}/carteira`).push({ ticker: acao.ticker, nomeEmpresa: acao.nome, quantidade: qtd, precoMedio: parseFloat(acao.preco) });
        }
      })
      ...
      .then(() => {
        ...
        Alert.alert('🎉 Compra realizada!', `Você comprou ${qtd} cotas de ${acao.ticker} por R$ ${total.toFixed(2)}\nSaldo restante: R$ ${novoSaldo.toFixed(2)}`,
          [{ text: 'Ver Carteira', onPress: () => this.props.onVerCarteira() }, { text: 'OK' }]);
      })
      ...
      <ScrollView style={estilos.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={estilos.cardAcao}>
          <View style={estilos.acaoTopo}>
            <View style={estilos.avatarGrande}>
              <Image source={{ uri: acao.logo }} style={estilos.logoImg} />
            </View>
            <View style={{ flex: 1 }}>
          ...
          <View style={estilos.precoRow}>
            ...
            <View style={estilos.precoItem}>
              ...
              <Text style={estilos.precoValor}>{Math.floor(this.state.saldoDisponivel / parseFloat(acao.preco))}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={estilos.formBox}>
          ...
          <View style={estilos.atalhoRow}>
            {[1, 5, 10, 50].map(n => (
              <TouchableOpacity key={n} style={estilos.btnAtalho} onPress={() => this.setState({ quantidade: String(n) })}>
          ...
          {qtd > 0 && (
            <View style={[estilos.resumoBox, { borderColor: saldoOk ? '#008b8b' : '#d63031' }]}>
              ...
              <View style={estilos.resumoLinha}>
                ...
                <Text style={[estilos.resumoValor, { color: saldoOk ? '#00b894' : '#d63031' }]}>R$ {saldoApos.toFixed(2)}</Text>
              </View>
              {!saldoOk && <Text style={estilos.erroSaldo}>⚠️ Saldo insuficiente</Text>}
            </View>
          )}
...
const estilos = StyleSheet.create({
  ...
  header: { backgroundColor: '#008b8b', paddingTop: 50, paddingBottom: 20, paddingHorizontal: 20, borderBottomLeftRadius: 24, borderBottomRightRadius: 24, marginBottom: 15 },
  ...
  input: { borderWidth: 1.5, borderColor: '#008b8b', borderRadius: 12, height: 55, paddingHorizontal: 16, fontSize: 20, color: '#333', marginBottom: 12 },
