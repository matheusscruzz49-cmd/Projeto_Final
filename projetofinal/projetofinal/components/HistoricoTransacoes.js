...
class HistoricoTransacoes extends React.Component {
  ...
  carregarHistorico() {
    ...
    firebase.database().ref(`usuarios/${usuario.uid}/transacoes`).orderByChild('data').once('value', snapshot => {
      const data = snapshot.val();
      const lista = data ? Object.keys(data).map(key => ({ uid: key, ...data[key] })).reverse() : [];
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            firebase.database().ref(`usuarios/${usuario.uid}/transacoes/${item.uid}`).remove()
              .then(() => this.setState(prev => ({ transacoes: prev.transacoes.filter(t => t.uid !== item.uid) })))
