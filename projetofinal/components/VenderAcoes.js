...
class VenderAcoes extends React.Component {
  ...
  confirmarVenda() {
    ...
    if (qtd > item.quantidade) {
      Alert.alert('Erro', `Você só tem ${item.quantidade} cotas de ${item.ticker}.`);
      return;
    }

    const total = this.calcularTotal();
    const lucroPerda = (precoAtual - item.precoMedio) * qtd;
    const lucroStr = lucroPerda >= 0
      ? `✅ Lucro: R$ ${lucroPerda.toFixed(2)}`
      : `❌ Prejuízo: R$ ${Math.abs(lucroPerda).toFixed(2)}`;
    ...
    Alert.alert(
      ...
      [
        ...
        { text: '💵 Confirmar Venda', onPress: () => this.executarVenda(qtd, total, lucroPerda) }
      ]
    );
  }
  ...
  executarVenda(qtd, total, lucroPerda) {
    ...
    firebase.database().ref(`usuarios/${uid}/transacoes`).push(transacao)
      .then(() => {
        ...
        if (qtdRestante <= 0) {
          // Remove completamente da carteira
          return firebase.database().ref(`usuarios/${uid}/carteira/${item.uid}`).remove();
        } else {
          // Atualiza quantidade (preço médio não muda na venda)
          return firebase.database().ref(`usuarios/${uid}/carteira/${item.uid}`).update({
      ...
      <ScrollView style={estilos.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={estilos.cardAcao}>
          ...
          <View style={estilos.precoRow}>
            <View style={estilos.precoItem}>
              <Text style={estilos.precoLabel}>Preço médio</Text>
              <Text style={estilos.precoValor}>R$ {item.precoMedio.toFixed(2)}</Text>
            </View>
            <View style={estilos.precoItem}>
              <Text style={estilos.precoLabel}>Preço atual</Text>
              <Text style={estilos.precoValor}>R$ {precoAtual.toFixed(2)}</Text>
            </View>
            <View style={estilos.precoItem}>
              <Text style={estilos.precoLabel}>Cotas em carteira</Text>
              <Text style={estilos.precoValor}>{item.quantidade}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={estilos.formBox}>
          ...
          <View style={estilos.atalhoRow}>
            {[
              { label: 'Tudo', val: item.quantidade },
              { label: '75%', val: Math.floor(item.quantidade * 0.75) },
              { label: '50%', val: Math.floor(item.quantidade * 0.5) },
              { label: '25%', val: Math.floor(item.quantidade * 0.25) },
            ].filter(b => b.val > 0).map(b => (
              <TouchableOpacity
                key={b.label}
                style={estilos.btnAtalho}
                onPress={() => this.setState({ quantidade: String(b.val) })}
              >
          ...
          {qtd > 0 && (
            <View style={[estilos.resumoBox, { borderColor: qtdValida ? '#d63031' : '#ccc' }]}>
              ...
              <View style={estilos.resumoLinha}>
                <Text style={estilos.resumoLabel}>Preço atual</Text>
                <Text style={estilos.resumoValor}>R$ {precoAtual.toFixed(2)}</Text>
              </View>
              ...
              {qtdValida && (
                <View style={[estilos.resumoLinha, { marginTop: 4 }]}>
                  ...
                  <Text style={[estilos.resumoValor, { color: pos ? '#00b894' : '#d63031', fontWeight: 'bold' }]}>
                    {pos ? '+' : '-'} R$ {Math.abs(lucroPerda).toFixed(2)}
                  </Text>
                </View>
              )}
              {!qtdValida && qtd > item.quantidade && (
                <Text style={estilos.erroQtd}>⚠️ Você só tem {item.quantidade} cotas</Text>
              )}
            </View>
          )}
          ...
          <TouchableOpacity
            ...
            onPress={() => this.confirmarVenda()}
            disabled={!qtdValida || this.state.carregando}
          >
            {this.state.carregando ? (
              <ActivityIndicator color="#fff" />
            ) : (
              <Text style={estilos.txtBotaoVender}>
                {qtdValida ? `💵 Vender ${qtd} cotas de ${item.ticker}` : 'Informe a quantidade'}
              </Text>
            )}
          </TouchableOpacity>
...
const estilos = StyleSheet.create({
  ...
  header: {
    backgroundColor: '#d63031', paddingTop: 50, paddingBottom: 20,
  ...
  avatarGrande: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#fdecea', justifyContent: 'center', alignItems: 'center' },
  avatarLetra: { color: '#d63031', fontWeight: 'bold', fontSize: 22 },
  ...
  input: {
    borderWidth: 1.5, borderColor: '#d63031', borderRadius: 12,
